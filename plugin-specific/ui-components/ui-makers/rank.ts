...
export class RankUIMaker extends ObjUIMaker<string> {
    override async MakeUI(
        ...
        const rankInput = itemDiv.createEl('input', { type: 'text', value: mainArray[index] } );

        this.MakeDeleteButton(view, itemDiv, mainArray, index, onRefresh);

        const updateRank = async (rank: Rank) => {
            mainArray[index] = rank.name;
            await onSave();
        };
        new ConceptSuggest(rankInput, view.life, undefined, view.app, updateRank, ['Skill Rank']);
    }
}