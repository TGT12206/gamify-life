...
export class ArrayEditor<T> {
    ...
    public async Render(view: ItemView) {
        ...
        const classNames = (this.isVertical ? 'vbox' : 'hbox') + ' gl-wide gl-scroll gl-outer-div ' + this.extraClasses;
    ...
    public async RefreshList(view: ItemView) {
        ...
        const isGrid = this.isGrid;
        if (this.isVertical) {
            if (isGrid) this.listDiv.style.setProperty('--num-rows', 'repeat(' + this.itemsPerLine + ', 1fr)');
        } else {
            if (isGrid) this.listDiv.style.setProperty('--num-columns', 'repeat(' + this.itemsPerLine + ', 1fr)');
        }
        ...
        for (let i = 0; i < displayedResults.length; i++) {
            ...
            const itemContainer = this.listDiv.createDiv('gl-outer-div ' + (this.itemUI.isVertical ? 'vbox' : 'hbox'));
